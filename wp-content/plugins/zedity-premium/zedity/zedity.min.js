...
(function($) {
    ...
    $.extend(Zedity.Box.Article.prototype, {
        createPropBar: function(options) {
                            ...
                            Articlebox: {
                                ...
                                features: {
                                    ...
                                    config: {
                                        ...
                                        order: 200,
                                        build: function($panel, ed) {
                                            $panel.append('<div class="zedity-article-quality-feat" style="border-bottom:1px solid silver;padding-bottom:10px;margin-bottom: 10px">' + '<span>' + Zedity.t('article quality') + '&nbsp;</span>' + '<select class="zedity-article-quality">' + '<option value="2">' + Zedity.t('Original') + '</option>' + '<option value="0.92">' + Zedity.t('High') + '</option>' + '<option value="0.88">' + Zedity.t('Normal') + '</option>' + '<option value="0.75">' + Zedity.t('Low') + '</option>' + '</select>' + '</div>' + '<span class="zedity-button zedity-articlepropresize">' + '<span class="zicon zicon-size-xs zicon-resize"></span> ' + Zedity.t('Proportional resize') + '</span><br/>' + '<span class="zedity-button zedity-articleoriginalsize">' + '<span class="zicon zicon-size-xs zicon-article"></span> ' + Zedity.t('Set box to article original size') + '</span>');
                                            $panel.find('.zedity-article-quality').selectmenu({
                                                width: 'auto',
                                                appendTo: $panel,
                                                change: function(e, ui) {
                                                    var box = ed.boxes.selected();
                                            ...
                                            $panel.find('.zedity-articlepropresize').on('click.zedity', function() {
                                                var box = ed.boxes.selected();
                                                if (!box) return;
                                            ...
                                            $panel.find('.zedity-articleoriginalsize').on('click.zedity', function() {
                                                var box = ed.boxes.selected();
                                        ...
                                        refresh: function(ed, box) {
                                            if (!box) return;
                                            this.$extpanel.find('.zedity-article-quality').val(box._options.quality).selectmenu('refresh');
                                            this.$extpanel.find('.zedity-articlepropresize').toggleClass('zedity-pressed', box.proportionalResize())
                                        },
                                        enable: function(ed, box) {
                                            return (box && box.$this.children('.zedity-empty').length == 0)
                                        }
            ...
            if ($('.zedity-dialog-article').length == 0) {
                var optionsElems = '';
                var cats = [];
                var catsElems = '';
                ...
                var $dialog = $(
                    '<div class="zedity-dialog-article">' +
                    '<div class="tabs">' +
                    '<ul>' +
                    '<li><a href="#tab-article-link">' + Zedity.t('Link') + '</a></li>' +
                    '<li><a href="#tab-article-disk">' + Zedity.t('Disk') + '</a></li>' +
                    '</ul>' +
                    '<div id="tab-article-disk">' + Zedity.t('Supported article file types:') + '<br/><span id="zedity-lblSupportedarticles"></span><br/><br/>' + Zedity.t('Select article file from disk (max size %s):', '<span id="zedity-lblMaxSize"></span>') + '<br/><br/>' +
                        ...
                        '<p id="zedity-lblarticleDisk" class="selected-file"></p>' + '<p class="zedity-article-quality">' + Zedity.t('article quality:') + ' ' + '<select id="zedity-ddarticleQuality" style="width:150px;">' + '<option value="2">' + Zedity.t('Original') + '</option>' + '<option value="0.92">' + Zedity.t('High') + '</option>' + '<option value="0.88" selected="selected">' + Zedity.t('Normal') + '</option>' + '<option value="0.75">' + Zedity.t('Low') + '</option>' + '</select>' + '</p>' +
                    '</div>' +
                    ...
                    '<textarea id="zedity-txtarticleLink" rows="4" style="display: none"></textarea>' +
                    '</div>' +
                    '<div id="tab-article-link">' + '<br/>' +
                    '</div>' +
                    '</div>' + '<p class="zedity-article-description" style="display: none">' + Zedity.t('article description:') + '<br/>' +
                    '<input id="zedity-txtArticleDescription" type="text" value="" style="display: none"><br/><br/>' + '</p>' +
                    '</div>');
                ...
                $dialog.dialog({
                    ...
                    open: function() {
                        ...
                        $this.find('input[type=text],textarea').val('');
                        $this.find('#zedity-ddarticleQuality').val(0.88);
                        $this.find('#zedity-lblMaxSize').text(Zedity.utils.beautifySize(box._options.maxSize));
                        $this.find('.tabs').tabs('option', {
                            active: 0,
                            disabled: disabled
                        });
                        $this.find('#zedity-lblSupportedarticles').text(Object.keys(Zedity.utils.fileTypes.types).join(', '));
                        var content = box.content();
                        if (content && /^http/.test(content)) {
                            $this.find('.tabs').tabs('selected', 'tab-article-link');
                            $this.find('#zedity-txtarticleLink').val(content)
                        }
                        $this.find('#zedity-txtArticleDescription').val(box.$this.find('img').attr('alt') || '')
                    },
                    close: function() {
                        var $this = $(this);
                        $('#zedity-lblarticleDisk').text('');
                    ...
                    buttons: [{
                        ...
                        click: function() {
                            ...
                            var box = $this.data('box');
                            if (box) {
                                var $inputDescription = $('#zedity-txtArticleDescription');
                                if (box._options.descriptionMandatory && $inputDescription.val().length < 4) {
                                    if ($inputDescription.val().length == 0) {
                                        box.editor._error({
                                            message: Zedity.t('Please insert article description.')
                                        })
                                    } else if ($inputDescription.val().length < 4) {
                                        box.editor._error({
                                            message: Zedity.t('article description is too short.')
                                        })
                                    }
                                ...
                                switch ($this.find('.tabs').tabs('selected')) {
                                    case 'tab-article-disk':
                                        ...
                                        box._options.quality = parseFloat($this.find('#zedity-ddarticleQuality').val());
                                        if (box._options.quality == 1) box._options.quality = 2;
                                        $fu.upload('submit');
                                        break;
                                    case 'tab-article-link':
                                        var content = $('#zedity-txtarticleLink').val();
                                        if (content == '') {
                                            box.editor._error({
            ...
            $('.zedity-dialog-article .zedity-fileupload').upload({
                autosubmit: false,
                ...
                onselect: $.proxy(function(event, file) {
                    var fn = $(file).val();
                    fn = fn.split(/[\\\/]/).pop();
                    $('#zedity-lblarticleDisk').html('Selected file:<br/><b>' + fn + '</b>');
                    $('#zedity-txtArticleDescription').focus()
                }, this),
                onsubmit: function(file) {
                    var $this = $(this);
                    var data = $this.data('upload');
                    var box = $this.parents('.zedity-dialog-article').data('box');
                    box._loading();
                    data.$input.trigger('mouseout.upload');
                    if (!Zedity.core.supports.fileapi()) {
                        box.editor._data.articleupload = box;
                        return true
                    }
                    Zedity.utils.resizearticle(file.files[0], {
                        maxwidth: box._sizeLimits().maxWidth,
                        maxheight: box._sizeLimits().maxHeight,
                        aspectratio: true,
                ...
                oncomplete: function(event, response) {
                    ...
                    if (response.status == 'OK') {
                        if (box.editor._data.articleupload) {
                            box.editor._data.articleupload.content(response.data);
                            delete box.editor._data.articleupload
                        } else {
                            box.editor._error({
                        ...
                        box.editor._error({
                            message: Zedity.t('There was an error during server article resize.')
                        })
                    }
                },
                onerror: function(error) {
        ...
        content: function(content, size, dontresize, onload) {
            ...
            if (content == null) {
                content = this.$this.find('img').css('background-article');
                if (/data:article\//.test(content)) {
                ...
                if (/^http/.test(content)) {
                    ...
                    $imgData.html('#' + this.editor.id + ' .imgDataS' + idx + ' img{background-article:url("' + content + '")}');
                    ...
                    this.editor._error({
                        message: Zedity.t('Could not interpret the content as article.')
                    })
                }
            }
            return content
        ...
        insert: function() {
            Zedity.Box.prototype.insert.call(this);
            $('.zedity-dialog-article').data('box', this).dialog('open');
            return this
        },
        duplicate: function() {
        ...
        _removeUnreferencedData: function() {
            var list = Zedity.core.gc.getNonReferenced('articleGc');
            for (var i = list.length - 1; i >= 0; --i) {
                $('style.' + list[i]).remove()
            }
            Zedity.core.gc.deleteReference(list);
        ...
        _addReference: function() {
            if (this.$this.hasNumberedClass('imgDataS')) {
                var nc = this.$this.getNumberedClass('imgDataS');
                Zedity.core.gc.addReference(nc, 'articleGc');
                Zedity.core.gc.addReference(nc, 'articleGc-' + this.id)
            }
            return this
        },
            ...
            if (nc) {
                ...
                if (style.length > 0) {
                    ...
                    if (['center', 'tile'].indexOf(this.layout()) > -1) {
                        ...
                        Zedity.utils.resizearticle(style.html(), {
                            maxwidth: bsize.w,
                            maxheight: bsize.h,
                            aspectratio: true,
                            ...
                            onresizeend: function(size, data, type) {
                                ...
                                box.editor.$this.children('.zedity-box-article.' + nc).removeClass(nc).addClass(newnc);
                                box._addReference();
